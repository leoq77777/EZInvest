...
async def test_chat_success(mock_run_agent, client):
...
@pytest.mark.asyncio
@patch("app.api.routes.chat.run_agent", new_callable=AsyncMock)
async def test_chat_agent_error(mock_run_agent, client):
...
async def test_chat_stream_format(mock_stream, client):
